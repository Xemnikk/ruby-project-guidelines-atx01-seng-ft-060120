class Game < ActiveRecord::Base
    ...
    def branch_progess
        if Main.prompt == continues
           branch = start_game[save_branch].index +=
           puts branch

            elsif Main.prompt == Right
                
                if branch start_game[save_branch] == :left_branch
                    puts start_game[:middle_branch].index +=
                else
                    puts start_game[:right_branch].index +=
                end

            elsif Main.prompt == Left
                
                if branch start_game[save_branch] == :middle_branch
                    puts start_game[:left_branch].index +=
                else
                    puts start_game[:middle_branch].index +=
                end
